...
n_people = [0,1]
def check_contingency(detection) -> str:
    global zero_people_printed
    n_people[0] = len(detection["predictions"]) #numero pessoas atual
    events = []
    if (n_people[0] == 0):
        if (n_people[1] > 0) and (n_people[1] <= 6):
            status = "low traffic"
        elif (n_people[1] > 6) and (n_people[1] <= 9):
            status = "normal traffic"
        elif (n_people[1] > 9) and (n_people[1] <= 12):
            status = "almost full"
        elif n_people[1] > 12:
            status = "full"
        else:
        ...
        if not zero_people_printed:
            for i in range(5):
                print(i)

            event_id = str(uuid4())
            events.append(EventStartedSignal(event_id, detection))
            events.append(EventEndedSignal(event_id, detection))

            print (f"AMOUNT OF PEOPLE: {n_people[1]} \nStatus: {status}") #saída no prompt
            n_people[1] = 0
            zero_people_printed = True

    else:
        zero_people_printed = False

    if (n_people[1] < n_people[0]):
        n_people[1] = n_people[0]
    print(n_people[1])
    return events
...
class RoIBusinessLogic(BusinessLogic):
    ...
    def _get_starting_events(self) -> List[EventStartedSignal]:
        ...
        for k, event in self._event_dict.items():
            if self._is_start_event(k, event):
                event_id = str(uuid4())
                events.append(EventStartedSignal(event_id, event.detection)) #retornar lista
    ...
    def process(self,
        ...
        if len(detection["predictions"]) < 5: #it shows in the front just the frames of locals with this number of amount people
